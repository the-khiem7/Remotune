...
// evtSubscribe starts a pull-mode subscription (no callback, no signal event; the
// caller polls with evtNext). Passing a bookmark with mode
// evtSubscribeStartAfterBookmark is the gap-free handover verified in Phase 0.
func evtSubscribe(channel, xpath string, bookmark evtHandle, flags uint32) (evtHandle, error) {
	cPath, err := windows.UTF16PtrFromString(channel)
	if err != nil {
		return 0, err
	}

...
**[VERIFIED]** The target-machine v0.1.5 run applied animation while CRD was active and recovery ownership was available. **[IMPLEMENTED]** v0.1.6 contains the quoted Windows Run-value comparison fix. **[VERIFIED]** v0.1.7 writes the quoted Windows Run entry, and its normalized path exists and matches the running executable. Subsequent target-machine acceptance confirmed the owned-snapshot Pause/Resume/Explicit Quit restoration sequence and autostart after sign-in.
...
9. Remotune exposes only the Performance Options Visual Effects radios required for CRD automation. On the accepted target-machine run, selecting `CRD ON → Custom` selects the profile but leaves the separate Remotune-owned Wails editor closed; the fixed `Custom visual effects` section's `Edit effects` action opens it beside the main popup, preferentially to its left. It does not replace the main popup or open the native Windows Performance Options dialog. The editor uses a familiar Visual Effects checklist model, keeps edits as a local draft, and crosses the backend/Windows boundary only when `Apply changes` submits the complete profile. It does not expose Advanced/Data Execution Prevention settings, generic Windows tuning, or Minimal/Recommended/Aggressive profiles. **[VERIFIED]** by target-machine observation. The source's automatic-open attempt is an implementation discrepancy, not a requirement.
...
78. **[VERIFIED]** Vue command feedback keeps a Pause/Resume error visible after refresh and acknowledges a no-snapshot pause. Tray Quit calls shutdown and restore before `app.Quit`, blocking the exit if restoration fails. The Wails/live-adapter restoration path was observed on the target machine with an owned snapshot: Pause restored, Resume reapplied, and Explicit Quit restored again.
79. **[VERIFIED]** `SetAutostart` writes a correctly quoted Windows Run value, but prior status comparison treated its outer quotes as part of the path and falsely reported a moved executable. Normalize the registry value by trimming optional outer quotes and whitespace before case-insensitive comparison. This is a display/status defect; it did not mean the registration path itself was wrong. Target-machine sign-in subsequently confirmed autostart launches.
80. **[VERIFIED]** The accepted target-machine behavior for Custom is explicit launch: selecting the Custom radio does not show the editor, while the fixed `Custom visual effects` section's `Edit effects` action does. `frontend/src/App.vue` at `42fd37d` still calls `OpenCustomEffectsEditor()` after Custom selection; do not silently reclassify that source/runtime discrepancy as either a failure or a product requirement without a follow-up decision.

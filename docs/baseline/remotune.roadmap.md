...
**[IMPLEMENTED]** Native Windows development is now the supported workflow. The pinned Wails CLI `v3.0.0-beta.8` is installed directly on the host, `build/config.yml` provides its dev-mode execution graph, and `wails3 dev` starts Vite HMR plus automatic Go rebuild/relaunch. The root `Taskfile.yml`, invoked through `wails3 task`, owns binding generation, reproducible frontend production build, Windows resource generation, verification, and portable versioned packaging. A full live hot-reload session remains **[UNVERIFIED]** until observed with the native tray application.
...
- **[IMPLEMENTED]** `out/remotune-v0.1.9.exe` was built successfully from the sole `build/config.yml` semantic version on 2026-08-25 (12,106,240 bytes; SHA-256 `d66fe4a3d978cba6da8c337de2261bcea71226ebbcacd1c7a2f11d745560c05b`). This is local packaging evidence, not target-machine acceptance.

...
`frontend/src/App.vue` translates numeric `crd.State` and `application.TuningState` values to display labels at its boundary. It chooses the main or Custom editor presentation from the Wails window query. The Custom editor copies persisted effects into a local draft, keeps polling from replacing a dirty draft, and sends one complete `SetProfileSettings` call only when the user selects `Apply changes`; `Revert` restores the persisted draft without a backend call. It must not call string operations on raw transport values. `frontend/src/wails.ts` remains the sole import point for generated bindings.
